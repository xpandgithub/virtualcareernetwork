...
(function($) {
	Drupal.behaviors.vcncma_college_credit = {
		attach: function(context, settings) {
			
			var collegecredits_data = Drupal.settings.vcncma_collegecredits.collegecredits_data;
                        var councelor_viewing_student_data = Drupal.settings.vcncma_collegecredits.councelor_viewing_student_data;
			var aaData = $.parseJSON(collegecredits_data);
			
			oTable = $('#cma-college-credit-listing').dataTable({
				...
				"aaData": aaData,
				"aaSorting": [[ 1, "asc" ]],
				"aoColumns": [{"sWidth": "70%","mData":"course_title","sType":"html"},{"sWidth": "20%","mData":"course_type"},{"sWidth": "10%", "bSortable": false,"mData":null}],
				"bAutoWidth": false,
				"bStateSave": false,
				"bDeferRender": true,
				...
				"fnRowCallback": function( nRow, aData, iDisplayIndex ) {
					$('td:eq(0)', nRow).html(aData.course_title);
                                        
                                        if (councelor_viewing_student_data == true) {
                                          $('td:eq(2)', nRow).html('<center><nobr>'+
                                                          '<input type="button" title="Delete" value="Delete" class="vcn-button grid-action-button vcn-button-disable" name="'+aData.usercourseid+'" />'+
                                                          '</nobr></center>');
                                        } else {
                                          $('td:eq(2)', nRow).html('<center><nobr>'+
                                                          '<input type="button" title="Delete" value="Delete" class="cma-college-credit-delete-this-row vcn-button grid-action-button" name="'+aData.usercourseid+'" />'+
                                                          '</nobr></center>');
                                        }
				}
			});
			
			$('#cma-college-credit-listing .cma-college-credit-delete-this-row').live('click', function(e) {
			  if (!$(this).hasClass('vcn-button-disable')) {	
				...
				var msg = 'Are you sure you want to remove selected course from your VCN Account?';
